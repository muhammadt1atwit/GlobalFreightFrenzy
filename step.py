...
def step(sim_state):
    ...
    # If no airports are configured, fall back to hubs (API spec says this happens automatically)
    if not airports:
        airports = hubs
    # If no ports are configured, ships cannot be used – we'll simply avoid spawning them.

    # Helper: check if a location is a valid operation facility for a vehicle type
    def can_operate_here(vtype, loc):
    ...
    def is_overseas(origin, dest):
        ...
        # For this simple algorithm, we will NOT send land vehicles on such routes.
        return dist > 500000
    ...
    for vid, v in vehicles.items():
        ...
        # Load boxes at current location (only if we are at a valid facility – optional, but safe)
        if capacity_left > 0 and can_operate_here(vtype, loc):
        ...
        if v["cargo"]:
            ...
            if vtype in [VehicleType.SemiTruck, VehicleType.Train] and is_overseas(loc, dest):
                # Avoid terrain penalty: redirect to nearest hub (maybe abandon the cargo)
                hub = min(hubs, key=lambda h: haversine_distance_meters(loc, h)) if hubs else None
                if hub:
                    sim_state.move_vehicle(vid, hub)
                else:
                    # No hub – just stay still (better than water crossing)
                    pass
            else:
    ...
    if tick == 0:
        ...
        for origin, bids in origin_boxes.items():
            # Separate overseas boxes (we will ignore them to avoid penalties)
            land_bids = [bid for bid in bids if not is_overseas(origin, boxes[bid]["destination"])]
            if not land_bids:
                continue  # skip this origin – all boxes would cause terrain penalty
